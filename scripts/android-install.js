...
module.exports = function (context) {
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android", "src", packageName.replace(/\./g, path.sep), "sgapi");
    ...
    engines.forEach(function(item,index) {
        if(item.name == 'android') {
            ...
            if (semver.gte(sepc,'7.0.0')) {
                ...
                targetDir  = path.join(projectRoot, "platforms", "android", "app", "src", "main", "java", packageName.replace(/\./g, path.sep), "sgapi");
            }
        }
    }); 

    var targetFiles = ["SGEntryActivity.java"];
    ...
    if (['after_plugin_add', 'after_plugin_install'].indexOf(context.hook) === -1) {
        ...
        targetFiles.forEach(function (f) {
            fs.readFile(path.join(context.opts.plugin.dir, 'src', 'android', f), {encoding: 'utf-8'}, function (err, data) {
                ...
                data = data.replace(/^package __PACKAGE_NAME__;/m, 'package ' + packageName + '.sgapi;');
